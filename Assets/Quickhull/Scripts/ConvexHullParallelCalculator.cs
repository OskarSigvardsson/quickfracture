...
namespace GK {
	public class ConvexHullParallelCalculator {
		...
		void GenerateInitialHull(List<Vector3> points) {
			...
			for (int i = 0; i < points.Count; i++) {
				if (i != vi0 && i != vi1 && i != vi2 && i != vi3) {
					pointsToReassign.Add(i);
				}
			}

			addedFaces.Clear();
			addedFaces.Add(new AddedFace { FaceIndex = 0, PointOnFace = points[faces[0].Vertex0], Normal = faces[0].Normal });
			addedFaces.Add(new AddedFace { FaceIndex = 1, PointOnFace = points[faces[1].Vertex0], Normal = faces[1].Normal });
			addedFaces.Add(new AddedFace { FaceIndex = 2, PointOnFace = points[faces[2].Vertex0], Normal = faces[2].Normal });
			addedFaces.Add(new AddedFace { FaceIndex = 3, PointOnFace = points[faces[3].Vertex0], Normal = faces[3].Normal });
		...
		void ReassignPoints(List<Vector3> points) {
			var reassignCount = pointsToReassign.Count;

			if (assignedFaces == null || assignedFaces.Length < reassignCount) {
				assignedFaces = new AssignedFace[Mathf.NextPowerOfTwo(reassignCount)];
			}

			Parallel.For(0, reassignCount, i => {
				var pointIndex = pointsToReassign[i];
				var point = points[pointIndex];

				assignedFaces[i] = new AssignedFace {
					Point    = pointIndex,
					Face     = UNASSIGNED,
					Distance = float.NaN,
				};
			...
			for (int i = 0; i < reassignCount; i++) {
				var af = assignedFaces[i];

				if (af.Face >= 0) {
					faces[af.Face].OpenSet.Add(new PointFace {
							Point = af.Point,
		...
		void VerifyMesh(List<Vector3> points, ref List<Vector3> verts, ref List<int> tris) {
			...
			Assert(tris.Count == 0 || tris.Count > (4 * 3));
